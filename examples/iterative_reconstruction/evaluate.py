"""
Iterative Reconstruction Evaluator with Autodiff Parameter Optimization

This module evaluates iterative reconstruction algorithms by measuring the
...
def compute_avg_r_bao_differentiable(recon_field: jnp.ndarray, gt_field: jnp.ndarray,
    ...
    """
    Compute average r(k) in a k range in a fully differentiable way.

    Args:
        recon_field: Reconstructed density field (3D)
        gt_field: Ground truth density field (3D)
        kmin: Minimum k (h/Mpc)
        kmax: Maximum k (h/Mpc)
...
class AutodiffParamOptimizer:
    ...
    def optimize(
        ...
        print(f"  Best score (avg r): {best_score:.6f}")
        print()
...
def get_kwargs(run_index: int, sim_start: int = 0) -> Dict[str, Any]:
    """Provides keyword arguments for reconstruction runs."""
...
# Large-scale r(k) baseline values (from initial algorithm)
# These are measured from the particle-based iterative_reconstruction_autodiff program
LARGE_SCALE_K_MIN = 0.01
LARGE_SCALE_K_MAX = 0.2
BASELINE_R_K_LARGE_SCALE = [
    0.9996,  # k bin 0 (k~0.022)
    0.9987,  # k bin 1 (k~0.047)
    0.9972,  # k bin 2 (k~0.071)
    0.9948,  # k bin 3 (k~0.096)
    0.9922,  # k bin 4 (k~0.120)
    0.9895,  # k bin 5 (k~0.145)
    0.9851,  # k bin 6 (k~0.169)
    0.9812,  # k bin 7 (k~0.194)
]
...
def aggregate_metrics(
    ...
    """Aggregates metrics across multiple reconstruction runs."""
    ...
    mean_r_k_large_scale = np.mean(all_r_k_large_scale, axis=0)
...
"""
    ...
    feedback = f"""
Iterative Reconstruction Performance (with Autodiff Optimization):
...
def main(
    ...
    """Runs the iterative reconstruction evaluation with autodiff parameter optimization."""
    total_start_time = time.time()

    if is_initial_program is None:
        is_initial_program = "/gen_0/" in results_dir or results_dir.endswith("/gen_0")

    log_header("ITERATIVE RECONSTRUCTION EVALUATION WITH AUTODIFF")
    print(f"  Timestamp: {timestamp()}")
    ...
    try:
        ...
        metrics = aggregate_metrics(all_results, results_dir)
...
if __name__ == "__main__":
    # MadEvolve dispatcher interface: python evaluate.py <candidate.py>
    if len(sys.argv) == 2 and not sys.argv[1].startswith("--"):
        ...
        parser = argparse.ArgumentParser(
            description="Iterative reconstruction evaluator with autodiff parameter optimization"
        )
